...
class EtfTrader:
    def __init__(self, state, prints, new_trader_data):
        ...
        self.baskets = [ProductTrader(s, state, prints, new_trader_data, product_group='ETF') for s in
                        ETF_BASKET_SYMBOLS]
        self.informed_constituent = ProductTrader(ETF_INFORMED_CONSTITUENT, state, prints, new_trader_data,
                                                  product_group='ETF')
        self.hedging_constituents = [ProductTrader(s, state, prints, new_trader_data, product_group='ETF') for s in
                                     ETF_CONSTITUENT_SYMBOLS if s != ETF_INFORMED_CONSTITUENT]
    ...
    def calculate_spread(self, basket):
        ...
        try:
            ...
            const_prices = [const.wall_mid for const in constituents.sort(
            ...
            if CALCULATE_RUNNING_ETF_PREMIUM:
                ...
                old_etf_mean_premium = self.last_traderData.get(f'ETF_{b_idx}_P',
                                                                [INITIAL_ETF_PREMIUMS[b_idx], n_hist_samples])
            ...
            old_etf_mean_premium = self.last_traderData.get(f'{basket.name[-1]}_P',
                                                            [INITIAL_ETF_PREMIUMS[b_idx], n_hist_samples])
    ...
    def get_basket_orders(self):
        ...
        for b_idx, basket in enumerate(self.baskets):
            ...
            if self.spreads[basket.name] > (
                BASKET_THRESHOLDS[b_idx] + informed_thr_adj) and basket.max_allowed_sell_volume > 0:
            ...
            elif self.spreads[basket.name] < (
                -BASKET_THRESHOLDS[b_idx] + informed_thr_adj) and basket.max_allowed_buy_volume > 0:
...
class OptionTrader:
    def __init__(self, state, prints, new_trader_data):
        ...
        self.options = [ProductTrader(os, state, prints, new_trader_data, product_group='OPTION') for os in
                        OPTION_SYMBOLS]
        self.underlying = ProductTrader(OPTION_UNDERLYING_SYMBOL, state, prints, new_trader_data,
        ...
        if self.underlying.wall_mid is not None:
            ...
            for option in self.options:
                ...
                if option.wall_mid is not None:
                    ...
                    new_mean_avg_dev = self.calculate_ema(f'{option.name}_avg_devs', IV_SCALPING_WINDOW,
    ...
    def get_iv_scalping_orders(self, options):
        ...
        for option in options:
            ...
            if option.name in self.indicators['mean_theo_diffs'] and option.name in self.indicators[
                'current_theo_diffs'] and option.name in self.new_switch_mean:
    ...
    def get_mr_orders(self, options):
        ...
        for option in options:
            ...
            if option.name in self.indicators['current_theo_diffs'] and option.name in self.indicators[
                'mean_theo_diffs'] and self.indicators.get('ema_o_dev') is not None:

                current_deviation = self.indicators['ema_o_dev']

                iv_deviation = self.indicators['current_theo_diffs'][option.name] - self.indicators['mean_theo_diffs'][
                    option.name]
    ...
    def get_option_orders(self):
        ...
        if self.state.timestamp / 100 < min(
            [THEO_NORM_WINDOW, underlying_mean_reversion_window, options_mean_reversion_window]): return {}
