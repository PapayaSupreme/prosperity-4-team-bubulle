# FROM v3.4 - implemented inventory skewing for quote price
# PnL : TBD
# SET             DAY    TICKS  OWN_TRADES    FINAL_PNL  RUN_DIR
# round3-carry    all    30000        1314    207223.00  runs/backtest-1777108155637
#
# PRODUCT             round3-carry
# HYDROGEL_PACK          114814.00
# VELVETFRUIT_EXTRACT     92409.00
import json
...
class Strategy:
    def __init__(self, symbol: Symbol, limit: int, anchor: int) -> None:
        self.symbol = symbol
        self.limit = limit
        self.anchor = anchor
...
class AdaptiveMarketMaker(StatefulStrategy):
    ...
    def __init__(self, symbol: Symbol, limit: int, anchor: int) -> None:
        super().__init__(symbol, limit, anchor)
        self.skew_strength = 1
        self.inventory_ratio_strength = 0.0 # best is 0 lol, but above would be safer
    ...
    def _compute_zscore(self, residual: float) -> None: # Use once !!
        ...
        self.z_score =  max(-self.z_score_clamp, min(self.z_score_clamp, z))

    def _estimate_fair_value(self, current_mid: float) -> float:
        residual = current_mid - self.anchor
    ...
    def act(self, state: TradingState) -> None:
        ...
        inventory_skew = self.inventory_ratio_strength * inventory_ratio

        fair_int = int(round(fair_value))
        bid_quote = min(best_bid + 1, fair_int - 1 - inventory_skew)
        ask_quote = max(best_ask - 1, fair_int + 1 + inventory_skew)
        ...
        anchor_signal = (current_mid - self.anchor) / 25 # completely random option but appears to work
...
class Trader:
    ...
    def __init__(self) -> None:
        ...
        self.strategies: dict[Symbol, Strategy] = {
            "HYDROGEL_PACK": AdaptiveMarketMaker("HYDROGEL_PACK", limits["HYDROGEL_PACK"], anchors["HYDROGEL_PACK"]),
            "VELVETFRUIT_EXTRACT": AdaptiveMarketMaker("VELVETFRUIT_EXTRACT", limits["VELVETFRUIT_EXTRACT"], anchors["VELVETFRUIT_EXTRACT"]),
        }
