# FROM v1.4 - fixes fallback reducing performance when not supposed to be triggered
# MC(toInt): NO_FALLBACK: 239 423 -> end_round(): 139 861 -> beginning_int(): 139 366 ->
    ...
    def act(self, state: TradingState) -> None:
        ...
        if self.use_fitted_fair_value:
            ...
            if fitted_fair_value is not None:
                ...
                fair_int = int(fitted_fair_value)
        else:
            ...
            if abs(fair_int - current_mid) > self.error_threshold:
                self.error_streak += 1
            else:
                self.error_streak = 0
